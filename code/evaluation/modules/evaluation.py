...
def report_cue(result_dict, cue, system = 'rule', colour = 'Purples'):
    pred = []
    gold = []
    ...
    labels = list(set(gold + pred))

    cm = confusion_matrix(gold, pred, labels=labels)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=labels)
    disp.plot(xticks_rotation=45, cmap = colour, colorbar = False)
    #plt.savefig(f'../../confusion-matrices/{cue}/matrix_{cue}_{system}.pdf', bbox_inches='tight')
    plt.show()


def report_cue_bar(result_dict, cue, system = 'rule', colour = 'purple'):
    pred = []
    gold = []
    ...
    # count predictions for items whose gold label is 'ambiguous'
    counts = {}
    ...
    categories = ['ambiguous', 'unambiguous_female', 'unambiguous_all', 'unambiguous_male']
    values = []
    ...
    fig, ax = plt.subplots()
    ax.bar(categories, values, color = colour)
    ax.set_ylabel('count')
    ax.set_xlabel('predicted label')
    plt.xticks(rotation = 45, ha = 'right')
    #plt.savefig(f'../../confusion-matrices/{cue}/bar_{cue}_{system}.pdf', bbox_inches='tight')
    plt.show()
...
def small_enough_for_spacy(text, max_chars=900000):
    chunks = []
...
def load_corpus_sources():
    ...
    for fileid in gutenberg.fileids():
        if fileid == 'bible-kjv.txt':
            continue
        sources.append((f"gutenberg/{fileid}", gutenberg.raw(fileid)))
    for fileid in inaugural.fileids():
        sources.append((f"inaugural/{fileid}", inaugural.raw(fileid)))
    for fileid in brown.fileids():
        text = ' '.join(brown.words(fileid))
        sources.append((f"brown/{fileid}", text))
    return sources


def clean_sentence(text):
    """
    collapses all whitespace (including tabs and newlines) to single
