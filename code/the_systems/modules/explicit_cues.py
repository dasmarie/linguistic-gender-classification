...
def detect_gender(text, target_index=None, model = None):
    """
...
def feature_gender(sentence, seed_token_index, clf, vectorizer, coref_model = coref_model, verbose=False):
    """
    classifies a single target noun in the context of a sentence.
    the target noun is specified by its token index in the spaCy doc.
    returns the predicted label, or None if the index is out of range.
    """
    spacy_doc = nlp(sentence)
    if seed_token_index is None or seed_token_index < 0 or seed_token_index >= len(spacy_doc):
        print('seed token index out of range:', seed_token_index)
        return None
    ...
    if verbose:
        ...
        for i in range(len(clf.classes_)):
            print(clf.classes_[i], round(probabilities[i], 3))
    return prediction

def bert_gender(tokens, cue_index, model, tokenizer):
    """
    ...
    """
    label_list = sorted(['ambiguous', 'unambiguous_all', 'unambiguous_female', 'unambiguous_male'])
    ...
    tokenized = temp_dataset.map(
        tokenize_and_align_labels,
        batched=True,
        fn_kwargs={'tokenizer': tokenizer, 'label_list': label_list}
    )
    predictions, _ = extract_cue_predictions(model=model, tokenized_dataset=tokenized,
...
def extract_cue_predictions(model, tokenized_dataset, og_examples, tokenizer, label_list):
    """
    ...
    for example in tokenized_dataset:
        ...
        raw_predictions.append(logits[0].numpy())
    predictions = list()
    for logits in raw_predictions:
...
def tokenize_and_align_labels(examples, tokenizer, label_list):
    """
    This function is copied from https://github.com/huggingface/notebooks/blob/main/examples/token_classification.ipynb
    """
